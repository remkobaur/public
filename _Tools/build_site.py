  ...
  <script>
    ...
    function buildRenderer(currentDoc) {
      ...
      renderer.image = ({ href, title, text }) => {
        ...
        const ttl = title ? ` title="${{title}}"` : "";
        return `<a href="${{src}}" target="_blank" rel="noopener"${{ttl}}><img src="${{src}}" alt="${{text || ""}}" loading="lazy"${{ttl}} /></a>`;
      };
    ...
    function rewriteRenderedAssetLinks(rootElement, currentDoc) {
      for (const image of rootElement.querySelectorAll("img[src]")) {
        ...
        if (parentLink) {
          const currentHref = (parentLink.getAttribute("href") || "").trim();
          if (!currentHref || !isExternalUrl(currentHref)) {
            parentLink.setAttribute("href", finalSrc);
    ...
    function moveCraftProjectDetailsIntoDescription(rootElement, currentDoc) {
      ...
      for (const table of rootElement.querySelectorAll("table")) {
        ...
        if (descIndex < 0 || detailsIndex < 0 || descIndex === detailsIndex) continue;
        ...
        for (const row of bodyRows) {
          ...
          if (cells.length <= Math.max(descIndex, detailsIndex)) continue;
          ...
          if (!detailsHtml || !detailsText) continue;
        ...
        for (const row of table.querySelectorAll("tr")) {
          const cells = [...row.children];
          if (cells.length > detailsIndex) {
            cells[detailsIndex].remove();
          }
        }
      }
    }
